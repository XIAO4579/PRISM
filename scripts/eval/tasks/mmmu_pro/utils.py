...
def mmmu_pro_doc_to_text(doc, lmms_eval_specific_kwargs=None):
    ...
    if "question" in doc and "options" in doc:
        ...
        question = "" if _CLEAN_PROMPT else post_prompt
    return question
...
def mmmu_pro_process_results(doc, results):
    ...
    if isinstance(pred, str):
        answer_tag_match = re.search(r"<answer>\s*([\s\S]*?)\s*</answer>", pred)
        if answer_tag_match:
            pred = answer_tag_match.group(1).strip()
    if "question" in doc and "options" in doc:
    ...
    formatted_question = construct_prompt(doc) if "question" in doc and "options" in doc else ""
    mmmu_acc = {"id": doc["id"], "subject": doc["subject"], "answer": doc["answer"], "parsed_pred": parsed_pred, "original_prediction": results[0], "formatted_question": formatted_question}
...
def evaluate_mmmu(samples):
    ...
    for sample in samples:
        if isinstance(sample["answer"], str) and isinstance(sample["parsed_pred"], str):
            ...
            # LLM judge fallback when format matching says wrong
            if not correct and "original_prediction" in sample and "formatted_question" in sample:
                try:
                    result = server.evaluate_binary(
                        question=sample["formatted_question"],
                        answer=str(gold_i),
                        prediction=sample["original_prediction"],
                        output_format="0/1",
                    )
                    if result["success"] and result["result"] == 1:
                        correct = True
