...
def mathvision_doc_to_text(doc, lmms_eval_specific_kwargs=None):
    ...
    query_prompt = "Please solve the problem step by step and put your final answer in <answer></answer> tags."
    if choices_str:
...
def mathvision_process_results(doc, results):
    ...
    for pred in results:
        ...
        answer_tag_match = re.search(r"<answer>\s*([\s\S]*?)\s*</answer>", model_answer)
        if answer_tag_match:
