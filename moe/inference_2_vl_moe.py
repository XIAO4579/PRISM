...
def process_sample(item, processor, image_base_path, max_length):
    ...
    def build_messages(response, include_image=True):
        ...
        if include_image and full_image_path:
            ...
            return messages, full_image_path
        else:
            # 纯文本格式：content 是字符串
            messages = [
...
def main():
    parser = argparse.ArgumentParser(description='Qwen2-VL MoE Value Head Inference')
    ...
    model = Qwen2VLMoeValueHead.from_pretrained(args.model_path)
