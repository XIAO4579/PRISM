...
from transformers.models.qwen3_moe.modeling_qwen3_moe import load_balancing_loss_func as qwen3_load_balancing_loss_func
from transformers.models.qwen2_vl_moe.modeling_qwen2_vl_moe import load_balancing_loss_func as qwen2_load_balancing_loss_func


class Qwen2VLMoeValueHead(nn.Module):
    """
    Qwen2-VL MoE 模型 + Value Head
    
    使用 Qwen2VLMoeForConditionalGeneration 作为 backbone，
    ...
    def __init__(
        ...
        # 加载 base model (Qwen2VLMoeForConditionalGeneration)
        ...
        self.v_head = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(hidden_size, num_labels, bias=False, dtype=torch.bfloat16)
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        Args:
            ...
            output_router_logits: 是否输出 router logits（用于计算 aux_loss）
        ...
        Returns:
            dict with:
                ...
                - router_logits: 如果 output_router_logits=True
                - aux_loss: MoE auxiliary loss（如果 output_router_logits=True）
        """
        # 获取 base model 的输出
        ...
        if output_router_logits:
            result['router_logits'] = outputs.router_logits if hasattr(outputs, 'router_logits') else None
    ...
    def save_pretrained(self, save_path: str):
        ...
        将 base_model 和 v_head 的权重合并保存，同时单独保存 value_head.safetensors
        以兼容 trl/verl 的加载方式。
        ...
        # 避免 DeepSpeed ZeRO 扁平化参数导致保存整个模型缓冲区
        v_head_trl_state = {}
        for k, v in v_head_state.items():
            if k == "1.weight":
        ...
        **kwargs
        ...
        if not os.path.exists(weights_path):
            ...
            if os.path.exists(index_path):
                weights_path = _load_v_head_from_sharded_checkpoint(model_path, index_path, model.v_head)
                if weights_path:
                    return model
        ...
        if os.path.exists(weights_path):
            ...
            if v_head_state:
                ...
                print(f"⚠️ No v_head weights found in {weights_path}, using random init")
        else:
            print(f"⚠️ No checkpoint found at {model_path}, using random init")
        
        return model


class Qwen3VLMoeValueHead(nn.Module):
    """
    Qwen3-VL MoE 模型 + Value Head
    
    使用 Qwen3VLMoeForConditionalGeneration 作为 backbone，
    添加一个 score head 用于输出 value/reward 分数。
    ...
    def __init__(
        ...
        # 加载 base model (Qwen3VLMoeForConditionalGeneration)
        self.base_model = AutoModelForImageTextToText.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16,
            trust_remote_code=False,
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        ...
        if gradient_checkpointing_kwargs is None:
            gradient_checkpointing_kwargs = {}
        # Pass as keyword argument, not unpacked
        self.base_model.gradient_checkpointing_enable(gradient_checkpointing_kwargs=gradient_checkpointing_kwargs)
    ...
    def forward(
        ...
        **kwargs
        ...
        if output_router_logits and hasattr(outputs, 'router_logits') and outputs.router_logits is not None:
            text_config = self.config.text_config if hasattr(self.config, 'text_config') else self.config
            aux_loss = qwen3_load_balancing_loss_func(
                outputs.router_logits,
                text_config.num_experts,
                text_config.num_experts_per_tok,
                attention_mask,
...
def _load_v_head_from_sharded_checkpoint(model_path: str, index_path: str, v_head: nn.Module) -> bool:
    ...
    try:
        ...
        if not v_head_files:
            print(f"⚠️ No v_head weights found in sharded checkpoint")
            return False
        ...
        print(f"⚠️ Failed to load v_head from sharded checkpoint: {e}")
        return False
