...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # if use black-box distillation
        teacher_response = None
        teacher_caption = None
        teacher_cot = None
        if 'teacher_response' in row_dict:
            teacher_response = row_dict.pop('teacher_response')
        if 'teacher_caption' in row_dict:
            teacher_caption = row_dict.pop('teacher_caption')
        if 'teacher_cot' in row_dict:
            teacher_cot = row_dict.pop('teacher_cot')
        ...
        # tokenize teacher_response if exists (for black-box distillation)
        if teacher_response is not None:
            ...
            max_response_length = getattr(self, 'max_response_length', 512)
        ...
        # moe black-box distillation
        if teacher_caption is not None and teacher_cot is not None:
            teacher_caption = self.tokenizer(teacher_caption, return_tensors="pt", add_special_tokens=False)
            teacher_cot = self.tokenizer(teacher_cot, return_tensors="pt", add_special_tokens=False)
            max_response_length = getattr(self, 'max_response_length', 512)
            if len(teacher_caption["input_ids"]) + len(teacher_cot["input_ids"]) > max_response_length:
                raise ValueError(f"Max response length is too short, {len(teacher_caption['input_ids'])} + {len(teacher_cot['input_ids'])} > {max_response_length}")
            teacher_caption_ids, _ = verl_F.postprocess_data(
                input_ids=teacher_caption["input_ids"],
                attention_mask=teacher_caption["attention_mask"],
                max_length=max_response_length,
                pad_token_id=self.tokenizer.pad_token_id,
                left_pad=False,
                truncation="right",
            )
            teacher_cot_ids, _ = verl_F.postprocess_data(
                input_ids=teacher_cot["input_ids"],
                attention_mask=teacher_cot["attention_mask"],
                max_length=max_response_length,
                pad_token_id=self.tokenizer.pad_token_id,
                left_pad=False,
                truncation="right",
            )
            teacher_caption_ids = teacher_caption_ids.long()
            teacher_cot_ids = teacher_cot_ids.long()
        ...
        # add teacher_response if exists (for black-box distillation)
        if teacher_response is not None:
            row_dict["teacher_response"] = teacher_response_ids[0]
        if teacher_caption is not None:
            row_dict["teacher_caption"] = teacher_caption_ids[0]
        if teacher_cot is not None:
            row_dict["teacher_cot"] = teacher_cot_ids[0]
