...
def _is_qwen3_vl_moe_model(model_config):
    """
    检测是否是 Qwen3-VL-MoE 模型（视觉语言 MoE）
    
    区分：
    - Qwen3-VL-MoE: model_type='qwen3_vl_moe', 有 text_config + vision_config
    - Qwen3-MoE (纯文本): model_type='qwen3_moe', 没有 text_config
    """
    model_type = getattr(model_config, 'model_type', '')
    
    # 方法1: 通过 model_type 精确匹配
    if model_type == 'qwen3_vl_moe':
        return True
    
    # 方法2: 同时有 text_config 和 vision_config，且 text_config 有 MoE 参数
    has_text_config = hasattr(model_config, 'text_config') and model_config.text_config is not None
    has_vision_config = hasattr(model_config, 'vision_config') and model_config.vision_config is not None
    
    if has_text_config and has_vision_config:
        text_config = model_config.text_config
        # text_config 可能是 dict 或 object
        if isinstance(text_config, dict):
            has_moe = 'num_experts' in text_config and 'num_experts_per_tok' in text_config
        else:
            has_moe = hasattr(text_config, 'num_experts') and hasattr(text_config, 'num_experts_per_tok')
        if has_moe:
            return True
...
def _is_qwen3_vl_dense_model(model_config):
    """
    检测是否是 Qwen3-VL Dense 模型（非 MoE 的视觉语言模型）

    匹配 model_type='qwen3_vl'，且不是 MoE 模型。
    """
    model_type = getattr(model_config, 'model_type', '')
    ...
    has_text_config = hasattr(model_config, 'text_config') and model_config.text_config is not None
    has_vision_config = hasattr(model_config, 'vision_config') and model_config.vision_config is not None
    if has_text_config and has_vision_config and not _is_qwen3_vl_moe_model(model_config):
        if 'qwen3' in model_type or 'qwen2_vl' in model_type:
            return True
...
def load_valuehead_model(local_path, torch_dtype, model_config, trust_remote_code):
    ...
    # === 优先检测 Qwen3-VL-MoE 模型 ===
    if _is_qwen3_vl_moe_model(model_config):
        print(f"[verl] Detected Qwen3-VL-MoE model, using Qwen3VLMoeValueHead")
        try:
            import sys
            import os
            moe_path = os.environ.get("MOE_MODULE_PATH")
            if moe_path and moe_path not in sys.path:
                sys.path.insert(0, moe_path)
            from value_head import Qwen3VLMoeValueHead
            
            model = Qwen3VLMoeValueHead.from_pretrained(
                local_path,
                num_labels=1,
                gradient_checkpointing=False,  # 推理时不需要
            )
            print(f"[verl] Successfully loaded Qwen3VLMoeValueHead from {local_path}")
            return model
        except Exception as e:
            print(f"[verl] Warning: Failed to load Qwen3VLMoeValueHead: {e}")
            print(f"[verl] Falling back to default loading method...")

    # === 检测 Qwen3-VL Dense 模型 (非 MoE 的视觉语言模型) ===
    if _is_qwen3_vl_dense_model(model_config):
        print(f"[verl] Detected Qwen3-VL Dense model, building Qwen3VLDenseValueHead")
        try:
            import os
            ...
            # 1. Import Qwen3VLDenseValueHead class (for its forward method)
            moe_path = os.environ.get("MOE_MODULE_PATH")
            dense_path = os.path.join(moe_path, "dense_model") if moe_path else None
            dense_value_head_path = os.path.join(dense_path, "value_head.py") if dense_path else None
            if not dense_value_head_path or not os.path.exists(dense_value_head_path):
                raise FileNotFoundError(
                    f"Qwen3VLDenseValueHead file not found: {dense_value_head_path}, "
                    f"please check MOE_MODULE_PATH={moe_path}"
                )
            import sys as _sys
            spec = importlib.util.spec_from_file_location("dense_value_head_module", dense_value_head_path)
            if spec is None or spec.loader is None:
                raise ImportError(f"Failed to build module spec for {dense_value_head_path}")
            dense_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(dense_module)
            _sys.modules["dense_value_head_module"] = dense_module
            Qwen3VLDenseValueHead = dense_module.Qwen3VLDenseValueHead

            # 2. Fix config: rope_parameters → rope_scaling (config JSON uses new name,
            #    but Qwen3VLTextConfig.__init__ only accepts rope_scaling)
            text_cfg = getattr(model_config, 'text_config', None)
            if text_cfg is not None:
                rope_params = getattr(text_cfg, 'rope_parameters', None)
                if rope_params is not None and getattr(text_cfg, 'rope_scaling', None) is None:
                    text_cfg.rope_scaling = rope_params
                    print(f"[verl] Patched text_config.rope_scaling from rope_parameters")

            # 3. Load base model via standard HF from_pretrained (works with init_empty_weights context)
            ...
            # 4. Build Qwen3VLDenseValueHead without calling its __init__ (which would conflict with meta tensors)
            ...
            model = Qwen3VLDenseValueHead.__new__(Qwen3VLDenseValueHead)
            nn.Module.__init__(model)
            model.config = model_config
            model.base_model = base_model
            model.num_labels = num_labels
            model.v_head = nn.Sequential(
                nn.Dropout(0.1),
                nn.Linear(hidden_size, num_labels, bias=True),
            )

            # 5. Load v_head weights from value_head.pt if available
            v_head_pt = os.path.join(local_path, "value_head.pt")
            if os.path.exists(v_head_pt):
                v_head_state = torch.load(v_head_pt, map_location="cpu", weights_only=True)
                seq_state = {}
                for k, v in v_head_state.items():
                    if k == "v_head.summary.weight":
                        seq_state["1.weight"] = v
                    elif k == "v_head.summary.bias":
                        seq_state["1.bias"] = v
                if seq_state:
                    model.v_head.load_state_dict(seq_state, assign=True)
                    print(f"[verl] Loaded v_head weights from {v_head_pt}")
            else:
                print(f"[verl] No value_head.pt found at {local_path}, v_head uses random init")

            print(f"[verl] Successfully built Qwen3VLDenseValueHead from {local_path}")
            return model
        except Exception as e:
            import traceback
            print(f"[verl] Warning: Failed to build Qwen3VLDenseValueHead: {e}")
            traceback.print_exc()
            print(f"[verl] Falling back to default loading method...")
    ...
    # Try to load v_head weights if they exist in local_path
    # This is necessary because trl's from_pretrained(model_instance) doesn't load weights from disk
    from transformers.utils import WEIGHTS_NAME, SAFE_WEIGHTS_NAME

    weights_path = os.path.join(local_path, SAFE_WEIGHTS_NAME)
    if not os.path.exists(weights_path):
        weights_path = os.path.join(local_path, WEIGHTS_NAME)
    ...
    if os.path.exists(weights_path):
        try:
            ...
            # Filter and load v_head weights
            v_head_state = {k.replace("v_head.", ""): v for k, v in state_dict.items() if k.startswith("v_head.")}

            if v_head_state:
                # Check compatibility with model.v_head
                # trl v_head is usually Sequential(Dropout, Linear) or just Linear
                if hasattr(model, "v_head"):
                    missing_keys, unexpected_keys = model.v_head.load_state_dict(v_head_state, strict=False)
                    if len(missing_keys) == 0:
                        print(f"Successfully loaded v_head weights from {weights_path}")
        except Exception as e:
            print(f"Warning: Failed to load v_head weights from {weights_path}: {e}")
