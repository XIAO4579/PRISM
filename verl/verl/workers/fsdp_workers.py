...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def compute_values(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            result = self.critic.compute_values(data=data)
            
            # Handle mm_gad mode which returns dict with separate scores
            if isinstance(result, dict):
                # Ensure caption_scores and cot_scores are 2D for DataProto chunk compatibility
                caption_scores = result["caption_scores"]
                cot_scores = result["cot_scores"]
                format_scores = result.get("format_scores", None)
                if caption_scores.dim() == 1:
                    caption_scores = caption_scores.unsqueeze(-1)  # (batch_size,) -> (batch_size, 1)
                if cot_scores.dim() == 1:
                    cot_scores = cot_scores.unsqueeze(-1)  # (batch_size,) -> (batch_size, 1)
                if format_scores is not None and format_scores.dim() == 1:
                    format_scores = format_scores.unsqueeze(-1)  # (batch_size,) -> (batch_size, 1)
                ...
                if format_scores is not None:
                    output_tensors["format_scores"] = format_scores
                output = DataProto.from_dict(tensors=output_tensors)
            else:
                output = DataProto.from_dict(tensors={"values": result})
