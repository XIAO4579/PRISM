...
class CriticConfig(BaseConfig):
    ...
    # whether to use gad mode (critic as discriminator for black-box distillation, whole response scoring)
    use_gad: bool = False
    # whether to use mm_gad mode (caption/cot separate scoring for multimodal)
    use_mm_gad: bool = False
    # optional score correction to reduce response-length bias in mm_gad scores
    use_score_length_correction: bool = False
    score_length_correction_beta: float = 0.0
    caption_length_correction_beta: Optional[float] = None
    cot_length_correction_beta: Optional[float] = None
    # optional L2 regularization on discriminator score magnitudes in mm_gad training
    score_reg_coef: float = 0.0
    # clamp critic scores to [-score_clip, score_clip] to prevent divergence
