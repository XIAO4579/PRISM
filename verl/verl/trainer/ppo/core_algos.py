...
class AdvantageEstimator(str, Enum):
    ...
    GAE = "gae"
    GRPO = "grpo"
    GRPO_MULTI_REWARD = "grpo_multi_reward"
...
@register_adv_est(AdvantageEstimator.GRPO_MULTI_REWARD)
def compute_grpo_multi_reward_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    caption_scores: Optional[torch.Tensor] = None,
    cot_scores: Optional[torch.Tensor] = None,
    acc_scores: Optional[torch.Tensor] = None,
    format_scores: Optional[torch.Tensor] = None,
    caption_weight: float = 0.5,
    cot_weight: float = 0.5,
    use_acc_gating: bool = False,
    use_gated_align: bool = False,
    use_rlvr: bool = False,
    use_rlvr_v2: bool = False,
    use_sign_align: bool = False,
    ...
    **kwargs,
    ...
    """
    Compute advantage for GRPO with multi-dimensional rewards.

    Modes (priority: use_sign_align > use_rlvr_v2 > use_rlvr > use_gated_align > use_acc_gating > default):

    Mode 1 (default):
        A = A_align * (1 - F) - format_penalty * F

    Mode 2 (use_acc_gating=True):
        A = align_coef * A_align - wrong_penalty * W - format_penalty * F

    Mode 3 (use_gated_align=True):
        A = align_coef * A_align * gate - wrong_penalty * W - format_penalty * F

    Mode 4 (use_rlvr=True):  GDPO/Constrained-GRPO style
        Step A: r_v = w_acc * acc + w_fmt * fmt  →  A_v = GN(r_v)
        Step B: A_align = w_cap * GN(r_cap) + w_cot * GN(r_cot)
        Step C: gate = (1 - F) * (1 - W)
        A = A_v + align_coef * A_align * gate

    Mode 5 (use_rlvr_v2=True):  RLVR v2 — penalties fused into r_v
        Step A: r_v = w_acc * acc + w_fmt * fmt - wrong_penalty * W - format_penalty * F  →  A_v = GN(r_v)
        Step B: A_align = w_cap * GN(r_cap) + w_cot * GN(r_cot)
        Step C: gate = (1 - F) * (1 - W)
        A = A_v + align_coef * A_align * gate

    Mode 6 (use_sign_align=True):  Sign-Reversed Alignment
        Step A: valid = 1 - F
                r_v = valid * (w_acc * acc + w_fmt * fmt - wrong_penalty * W) - format_penalty * F  →  A_v = GN(r_v)
        Step B: A_align = w_cap * GN(r_cap) + w_cot * GN(r_cot)
        Step C: sign_factor = (1 - F) * (1 - 2 * W)   # +1 / -1 / 0
        A = A_v + align_coef * A_align * sign_factor
    """
    if caption_scores is None or cot_scores is None:
        raise ValueError("caption_scores and cot_scores must be provided for grpo_multi_reward")

    def _group_normalize(scores: torch.Tensor, index: np.ndarray, eps: float) -> torch.Tensor:
        """Normalize scores within each group (GRPO-style)."""
        id2score = defaultdict(list)
        id2mean = {}
        id2std = {}
    ...
    with torch.no_grad():
        # Compute format_invalid FIRST so we can adjust scores before normalization
        if format_scores is not None:
            format_invalid = (format_scores <= 0).float()
        else:
            format_invalid = torch.zeros_like(caption_scores)

        # For format-invalid samples, replace their 0-scores with
        # (group_min_of_valid_samples - 1) so they sit below all valid
        # samples and don't pollute the group normalization.
        cap_adj = caption_scores.clone()
        cot_adj = cot_scores.clone()
        bsz = caption_scores.shape[0]
        id2valid_cap_min = {}
        id2valid_cot_min = {}
        for i in range(bsz):
            if format_invalid[i] == 0:
                gid = index[i]
                cv = caption_scores[i].item()
                if gid not in id2valid_cap_min or cv < id2valid_cap_min[gid]:
                    id2valid_cap_min[gid] = cv
                tv = cot_scores[i].item()
                if gid not in id2valid_cot_min or tv < id2valid_cot_min[gid]:
                    id2valid_cot_min[gid] = tv
        for i in range(bsz):
            if format_invalid[i] == 1:
                gid = index[i]
                cap_adj[i] = id2valid_cap_min.get(gid, 0.0) - 1.0
                cot_adj[i] = id2valid_cot_min.get(gid, 0.0) - 1.0

        # (B) Alignment advantage from critic (MoE discriminator)
        norm_cap = _group_normalize(cap_adj, index, epsilon)
        norm_cot = _group_normalize(cot_adj, index, epsilon)
        a_align = caption_weight * norm_cap + cot_weight * norm_cot

        if acc_scores is not None:
            wrong = (acc_scores <= 0).float() * (1 - format_invalid)
        else:
            wrong = torch.zeros_like(a_align)

        if use_sign_align:
            # Mode 6: Sign-Reversed Alignment
            _acc = acc_scores if acc_scores is not None else torch.zeros_like(a_align)
            _fmt = format_scores if format_scores is not None else torch.ones_like(a_align)
            valid_format = 1 - format_invalid
            r_v = (valid_format * (w_acc * _acc + w_fmt * _fmt - wrong_penalty * wrong)
                   - format_penalty * format_invalid)
            a_v = _group_normalize(r_v, index, epsilon)

            sign_factor = (1 - format_invalid) * (1 - 2 * wrong)
            total_adv = a_v + align_coef * a_align * sign_factor
        elif use_rlvr_v2:
            # Mode 5: RLVR v2 — wrong_penalty & format_penalty fused into r_v
            _acc = acc_scores if acc_scores is not None else torch.zeros_like(a_align)
            _fmt = format_scores if format_scores is not None else torch.ones_like(a_align)
            r_v = (w_acc * _acc + w_fmt * _fmt
                   - wrong_penalty * wrong
                   - format_penalty * format_invalid)
            a_v = _group_normalize(r_v, index, epsilon)

            gate = (1 - format_invalid) * (1 - wrong)
            total_adv = a_v + align_coef * a_align * gate
        elif use_rlvr:
            # Mode 4: RLVR / GDPO style
            # (A) Verifiable reward: r_v = w_acc * acc + w_fmt * fmt → A_v = GN(r_v)
            _acc = acc_scores if acc_scores is not None else torch.zeros_like(a_align)
            _fmt = format_scores if format_scores is not None else torch.ones_like(a_align)
            r_v = w_acc * _acc + w_fmt * _fmt
            a_v = _group_normalize(r_v, index, epsilon)

            # (C) A_v provides the base signal; A_align gated to feasible set only
            gate = (1 - format_invalid) * (1 - wrong)
            total_adv = a_v + align_coef * a_align * gate
        elif use_gated_align:
            # Mode 3: gated alignment
            gate = (1 - format_invalid) * (1 - wrong)
            total_adv = (align_coef * a_align * gate
                         - wrong_penalty * wrong
                         - format_penalty * format_invalid)
        elif use_acc_gating:
            # Mode 2: acc gating
            total_adv = (align_coef * a_align
                         - wrong_penalty * wrong
                         - format_penalty * format_invalid)
        else:
            # Mode 1: alignment + explicit format penalty
            total_adv = a_align - format_penalty * format_invalid

        advantages = total_adv.unsqueeze(-1) * response_mask
...
def compute_discriminator_loss(student_vpreds: torch.Tensor, teacher_vpreds: torch.Tensor, response_mask: torch.Tensor, teacher_response_mask: torch.Tensor):
    """Compute discriminator loss for mm_gad black-box distillation.
    
    Args:
        student_vpreds: Value predictions for student responses
        teacher_vpreds: Value predictions for teacher responses
        response_mask: Mask for student responses
        teacher_response_mask: Mask for teacher responses
        
    Returns:
        d_loss: Discriminator loss (negative log sigmoid of teacher-student reward difference)
    """
