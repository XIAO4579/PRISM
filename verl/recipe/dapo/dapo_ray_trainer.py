    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                for reward_key in ("acc_reward", "format_reward"):
                    if reward_key in batch.non_tensor_batch:
                        vals = batch.non_tensor_batch[reward_key]
                        metrics[f"reward/{reward_key}/mean"] = float(np.mean(vals))
                        metrics[f"reward/{reward_key}/max"] = float(np.max(vals))
                        metrics[f"reward/{reward_key}/min"] = float(np.min(vals))
